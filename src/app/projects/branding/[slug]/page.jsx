...
async function getImages(slug) {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URI}/v1_1/${cloudName}/resources/image?type=upload&prefix=photo/aplus/${slug}&max_results=50`, {
    headers: {
  ...
  return (
    <div className='w-full py-12 bg-white'>
      <div className='w-full md:w-[65%] px-4 md:px-20 flex flex-col gap-3 items-start'>
        ...
        <p className='text-md md:text-lg leading-relaxed text-justify text-zinc-500'>{branding?.content}</p>
        <p className='text-zinc-500 text-md italic pt-6'>Berikut adalah beberapa foto project <span className='text-zinc-400 text-sm md:text-md capitalize font-semibold not-italic'> {branding?.title}</span> yang sudah kami kerjakan :</p>
      </div>
      ...
      <div className='w-fit mx-auto'>
        <Link href="/projects/branding" className='group' passHref>
          <button type='button' className='group text-zinc-500 flex items-center justify-center gap-2 px-5 py-2 group-hover:text-zinc-600  transition-all duration-300 ease-linear'>
